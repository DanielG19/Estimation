...
void QuadEstimatorEKF::UpdateFromIMU(V3F accel, V3F gyro)
{
  ...
  //ekfState(6) = ekfState(6) + dtIMU * gyro.z;	// yaw
    ...
    Quaternion<float> quatTransformation = Quaternion<float>::FromEuler123_RPY(rollEst, pitchEst, ekfState(6));

    //Quat transform for pitch and roll
    quatTransformation.IntegrateBodyRate(gyro, dtIMU);
    float Hat_Roll = quatTransformation.Roll();
    float Hat_Pitch = quatTransformation.Pitch();
    ekfState(6) = quatTransformation.Yaw();
  ...
  rollEst = attitudeTau / (attitudeTau + dtIMU) * (Hat_Roll)+dtIMU / (attitudeTau + dtIMU) * accelRoll;
  pitchEst = attitudeTau / (attitudeTau + dtIMU) * (Hat_Pitch)+dtIMU / (attitudeTau + dtIMU) * accelPitch;
...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
  ...
  //Accelerometers doesnt measure gravity, 0 = free falling
  V3F attitude_Rates = attitude.Rotate_BtoI(accel) + V3F(0, 0, -9.81f);

  predictedState(0) += ekfState(3) * dt;
  predictedState(1) += ekfState(4) * dt;
  predictedState(2) += ekfState(5) * dt;
  predictedState(3) += attitude_Rates.x * dt;
  predictedState(4) += attitude_Rates.y * dt;
  predictedState(5) += attitude_Rates.z * dt;
...
MatrixXf QuadEstimatorEKF::GetRbgPrime(float roll, float pitch, float yaw)
{
  ...
  RbgPrime(0, 1) = -cos_Phi * sin_Theta * sin_Psi + sin_Phi * cos_Psi;
...
void QuadEstimatorEKF::Predict(float dt, V3F accel, V3F gyro)
{
  ...
  ////////////////////////////// BEGIN STUDENT CODE ///////////////////////////
  //gPrime already have the states, thus only need to get the controls_ut and the ComplementedRbgMatrix
  MatrixXf controls_ut(3, 1);
  controls_ut(0, 0) = accel.x * dt;
  controls_ut(1, 0) = accel.y * dt;
  controls_ut(2, 0) = accel.z * dt;

  //Creating the jacobian
  MatrixXf ComplementedRbgMatrix = RbgPrime * controls_ut;

  gPrime(0, 3) = dt;
  gPrime(1, 4) = dt;
  gPrime(2, 5) = dt;
  gPrime(3, 6) = ComplementedRbgMatrix(0, 0);
  gPrime(4, 6) = ComplementedRbgMatrix(1, 0);
  gPrime(5, 6) = ComplementedRbgMatrix(2, 0);
...
void QuadEstimatorEKF::UpdateFromMag(float magYaw)
{
  ...
  float Z_delta = magYaw - zFromX(0);

  // normalizing to -180 to 180
  if (Z_delta > F_PI) Z_delta -= 2.f * F_PI;
  if (Z_delta < -F_PI) Z_delta += 2.f * F_PI;

  //Estimation for new measurement.
  zFromX(0) = magYaw - Z_delta;
